...
exports.findAll = (req, res) => {
  ...
  const filter = {
    ...
    confirmed_status:
      confirmed_status == "true" ? "1" : confirmed_status == "false" ? "0" : { [Op.ne]: null },
  };
  ...
  Payment.findAndCountAll({
    ...
    .catch((error) => {
      res.status(500).json({
        message: `Tidak berhasil menampilkan data Payment, ${error.message}`,
      });
    });
};
...
exports.findStudentPacket = (req, res) => {
  ...
  Payment.findAll({
    ...
    .catch((err) => {
      res.status(500).json({
        message:
          "Tidak berhasil menampilkan data packet dengan studentId => " +
          studentIds,
      });
    });
};
...
exports.create = (req, res) => {
  ...
  Payment.create(payment)
    .then((payments) => {
      res.json({
        message: "Record Payment berhasil ditambahkan",
        data: payments,
      });
    })
    .catch((err) => {
      res.status(500).json({
        message: `Tidak berhasil menambah data payment, error : ${err.message}`,
      });
    });
};
  ...
  Payment.update(req.body, {
    ...
    .then((result) => {
      if (result == 1) {
        ...
        res.status(500).json({
          message: "Tidak berhasil update payment dengan id => " + id,
        });
      }
    })
    .catch((err) => {
      res
        .status(500)
        .json({ message: "Tidak berhasil update payment dengan id => " + id });
    });
};
  ...
  Payment.destroy({
    ...
    .then((result) => {
      if (result == 1) {
        ...
        res
          .status(500)
          .json({ message: "Tidak berhasil hapus payment dengan id => " + id });
      }
    })
    .catch((err) => {
      res
